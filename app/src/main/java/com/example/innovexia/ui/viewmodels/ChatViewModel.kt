...
class ChatViewModel(
    ...
    private val firebaseRateLimiter: com.example.innovexia.core.ratelimit.FirebaseRateLimiter
) : ViewModel() {
    ...
    fun sendMessage(text: String, persona: Persona? = null) {
        ...
        viewModelScope.launch {
            try {
                ...
                val (canSend, retrySeconds) = firebaseRateLimiter.canMakeRequest(plan)

                if (!canSend) {
                    android.util.Log.w("ChatViewModel", "Rate limit exceeded - must wait $retrySeconds seconds")
                    _errorMessage.value = "Rate limit exceeded. Please wait $retrySeconds seconds."
                    startCountdown(retrySeconds)
                    isSending = false
                    return@launch
                }

                android.util.Log.d("ChatViewModel", "Rate limit check passed - proceeding with message send")
                ...
                streamingJob = streamScope.launch {
                    ...
                    try {
                        ...
                        android.util.Log.d("ChatViewModel", "Rate limiter: Request recorded")
